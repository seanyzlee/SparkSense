...
const Create = async (req, res, next) => {
    try {
        for (let i = 0; i < req.body.length; i++) {
            try {
                const newWeatherInfo = new ONWeatherInfo(req.body);
...
const Read = async (req, res, next) => {
    // Read - GET /on
    try {
        const weatherData = await ONWeatherInfo.find()
...
const PostONInfo = async (req, res, next) => {
    try {
        ...
        const result = await ONWeatherInfo.insertMany(weatherData);
