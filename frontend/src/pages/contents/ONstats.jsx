...
const ONstats = () => {
    ...
    const response = await axios.get("http://localhost:5050/on")
    setCity("Haliburton Forest & Wild Life Reserve Ltd")
    let times = []
    let temperatures = []
    let precipitations = []
    let cloudCovers = []
...
if (loading) {
    ...
    return (
        ...
        <div>
             <Title level={2} style={{textAlign: 'center'}}>Haliburton Forest & Wild Life Reserve Ltd</Title>
        ...
        {percent < 15 && (
            <div>
                <Title level={4} style={{textAlign: 'center', color: 'green'}}>No Warnings</Title>
            </div>
        )}
